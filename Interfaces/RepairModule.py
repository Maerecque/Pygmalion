...
class App:
    ...
    def plane_detection(self):
        try:
            self.plane_pointcloud = walk_through_grid(self.point_cloud_data, self.grid, self.min_cell_size.get(), self.min_plane_size.get())
            self.plane_detection_result_label.config(text=f"Plane detection completed.\n{len(self.plane_pointcloud)} planes found.")
    ...
    def create_window(self):
        ...
        tk.Label(grid_division_frame, text="Grid Size").grid(row=0, column=0, padx=5, pady=5)
        tk.Entry(grid_division_frame, textvariable=self.grid_size).grid(row=0, column=1, padx=5, pady=5)
        tk.Label(grid_division_frame, text="Grid Overlap").grid(row=1, column=0, padx=5, pady=5)
        tk.Entry(grid_division_frame, textvariable=self.grid_overlap).grid(row=1, column=1, padx=5, pady=5)

        self.grid_division_button = tk.Button(grid_division_frame, text="Start Grid Division", command=self.start_grid_division_thread)
        self.grid_division_button.grid(row=1, column=2, padx=5, pady=5)

        self.grid_devision_result_label = tk.Label(grid_division_frame, text="Grid not divided yet.")
        self.grid_devision_result_label.grid(row=0, column=2, padx=5, pady=5)
        ...
        tk.Label(plane_detection_frame, text="Min Cell Size").grid(row=0, column=0, padx=5, pady=5)
        self.min_cell_size_entry = tk.Entry(plane_detection_frame, textvariable=self.min_cell_size, state=tk.DISABLED)
        self.min_cell_size_entry.grid(row=0, column=1, padx=5, pady=5)
        tk.Label(plane_detection_frame, text="Min Plane Size").grid(row=1, column=0, padx=5, pady=5)
        self.min_plane_size_entry = tk.Entry(plane_detection_frame, textvariable=self.min_plane_size, state=tk.DISABLED)
        self.min_plane_size_entry.grid(row=1, column=1, padx=5, pady=5)

        self.plane_detection_button = tk.Button(plane_detection_frame, text="Start Plane Detection", state=tk.DISABLED, command=self.start_plane_detection_thread)
        self.plane_detection_button.grid(row=1, column=2, padx=5, pady=5)

        self.plane_detection_result_label = tk.Label(plane_detection_frame, text="Planes not detected yet.")
        self.plane_detection_result_label.grid(row=0, column=2, padx=5, pady=5)
        ...
        tk.Label(repair_point_cloud_frame, text="Kdtree Max NN").grid(row=0, column=0, padx=5, pady=5)
        self.kdtree_max_nn_entry = tk.Entry(repair_point_cloud_frame, textvariable=self.kdtree_max_nn, state=tk.DISABLED)
        self.kdtree_max_nn_entry.grid(row=0, column=1, padx=5, pady=5)
        tk.Label(repair_point_cloud_frame, text="Depth").grid(row=1, column=0, padx=5, pady=5)
        self.depth_entry = tk.Entry(repair_point_cloud_frame, textvariable=self.depth, state=tk.DISABLED)
        self.depth_entry.grid(row=1, column=1, padx=5, pady=5)
        tk.Label(repair_point_cloud_frame, text="Quantile Value").grid(row=2, column=0, padx=5, pady=5)
        self.quantile_value_entry = tk.Entry(repair_point_cloud_frame, textvariable=self.quantile_value, state=tk.DISABLED)
        self.quantile_value_entry.grid(row=2, column=1, padx=5, pady=5)
        tk.Label(repair_point_cloud_frame, text="Scale").grid(row=0, column=2, padx=5, pady=5)
        self.scale_entry = tk.Entry(repair_point_cloud_frame, textvariable=self.scale, state=tk.DISABLED)
        self.scale_entry.grid(row=0, column=3, padx=5, pady=5)

        self.repair_point_cloud_button = tk.Button(repair_point_cloud_frame, text="Start Point Cloud Repair", state=tk.DISABLED, command=self.start_repair_point_cloud_thread)
        self.repair_point_cloud_button.grid(row=1, column=2, columnspan=2, padx=5, pady=5)

        self.repair_point_cloud_result_label = tk.Label(repair_point_cloud_frame, text="Point cloud not repaired yet.")
        self.repair_point_cloud_result_label.grid(row=2, column=2, columnspan=2, padx=5, pady=5)
        ...
        self.save_button = tk.Button(save_frame, text="Save Repaired Point Cloud", state=tk.DISABLED, command=self.save_repaired_point_cloud)
        self.save_button.grid(row=0, column=0, padx=5, pady=5)

        self.show_button = tk.Button(save_frame, text="Show Repaired Point Cloud", state=tk.DISABLED, command=lambda: open_point_cloud_editor(self.mesh, False) if self.mesh else print("No repaired point cloud to show."))
        self.show_button.grid(row=0, column=1, padx=5, pady=5)

        # Exit Button
        exit_button = tk.Button(main_frame, text="Back", command=self.root.destroy)
