...
def create_alpha_shape(
    ...
    # # Ball pivoting
    # # Seems to be very slow, even on small point clouds
    # # I don't even know if this works
    # print("Computing normal and rendering it.")
    # input_point_cloud.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(knn=30), fast_normal_computation=False)
    # print("Starting for loop.")
    # for radii in np.logspace(np.log10(0.2), np.log10(0.001), num=20):
    #     print(f"Computing mesh for radii: {radii}")
    #     mesh = o3d.geometry.TriangleMesh.create_from_point_cloud_ball_pivoting(input_point_cloud, o3d.utility.DoubleVector([radii, radii * 2]))
    #     print(f"Computing mesh for radii: {radii} done.")
    #     if visualize:
    ...
    copy_input_point_cloud = copy.deepcopy(input_point_cloud)

    # Downsample the point cloud
    # Notes: A lower radius value shows more detail, but takes longer to compute
    # This is not really needed, since we will always downsample the point cloud before running this function
    # copy_input_point_cloud.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.001, max_nn=30))
    o3d.visualization.draw_geometries([copy_input_point_cloud], left=0, top=45)

    # Higher scale, more fill up, but also more random guessing and more "detail"
    mesh, densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(copy_input_point_cloud, depth=8, width=0, scale=2.2, linear_fit=True)
    ...
    # # Alpha shapes
    # for alpha1 in np.logspace(np.log10(2), np.log10(0.001), num=20):
    #     # This line doesn't work for some reason
    #     mesh = o3d.geometry.TriangleMesh.create_from_point_cloud_alpha_shape(input_point_cloud, alpha1)
    #     mesh.compute_vertex_normals()
    #     print(f"alpha={alpha1:.3f}")
    #     if visualize:
    #         o3d.visualization.draw_geometries([mesh], left=0, top=45)
